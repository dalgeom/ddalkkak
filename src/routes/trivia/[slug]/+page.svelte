<script lang="ts">
	...
	const desc = $derived(
		`${data.category.title} ${data.items.length}문제를 정답과 해설까지 한 페이지에 모았습니다. 가입도 결제도 없이 무료로 보고, 무한 연습에서 바로 풀어볼 수 있습니다.`
	);
</script>

<svelte:head>
	<title>{heading} — 정답·해설 포함 | 딸깍</title>
	<meta name="description" content={desc} />
	<link rel="canonical" href={url} />
	<meta property="og:title" content="{heading} — 정답·해설 포함 | 딸깍" />
