 ...
 */
const 알려진_적발: string[] = [
	// ── 비어 있다. 2026-09-08에 격간(자리 홀짝) 취약을 정리했다.
	//
	// 9/07 스레드 댓글이 rc-chain-maxdigit(27 → 20 → 18 → 10 → ?)에서 이 종류를 열었다.
	// 「홀수 자리 27·18은 공차 −9로도 공비 2/3로도 읽힌다」 — 맞는 말이었다.
	// 검사기에 넣었더니 수열·사슬 15개 중 7개가 걸렸는데, 그중 넷은 검사기 쪽 결함이었다.
	//
	// **사람이 격간을 세우려면 양쪽 갈래가 다 진행이어야 한다.** 한쪽만 보면 거짓 적발이 난다.
	//   nm-square-reverse  반대쪽 1·9·52  — 등차도 등비도 아니다 → 사람은 이 가설을 버린다
	//   binary-seq         반대쪽 1·11·101
	//   disc-6             반대쪽 2004·4016 쪽은 되지만 물음표 쪽 1001·3009·5025가 진행이 아니다
	//   nm-seven-ones      반대쪽 7·1·5
	// 반대쪽 검사를 넣어 넷을 기각했다. 양성 대조로 rc-chain-maxdigit(12)·rc-chain-sqsum(187)은
	// 여전히 잡히는 것을 확인했다 — 넷이 사라진 것이 검사기가 죽어서가 아니다.
	//
	// 나머지 셋은 지문에 「앞의 수로 다음 수를 만든다」를 넣어 고쳤다. 화살표든 나열이든
	// 규칙이 실제로 단계 함수면 그렇게 못 박는 것이 맞고, 검사기도 그 지문을 보면 면제한다.
	//   rc-chain-maxdigit · rc-chain-sqsum · nm-grow-mult
