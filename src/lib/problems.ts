...
export const PROBLEMS: Problem[] = [
	...
	},
	{
		...
		chip: '오름차순',
		blocks: [
			{ kind: 'text', html: '여섯 단어 중 <b>하나만</b> 나머지와 다르다. 어느 것일까?' },
			{ kind: 'pre', text: 'ALMOST\nBEGIN\nCHIMP\nGHOST\nEMPTY\nGIFT' }
		],
		type: 'text',
		answers: ['GIFT'],
		hints: [
			'뜻도 길이도 아닙니다. 글자가 <b>나열된 순서</b>를 보세요.',
			'각 글자에 A=1, B=2 … Z=26으로 번호를 매겨 적어 보세요.',
			'다섯 단어는 번호가 커지기만 합니다. 하나는 도중에 줄어듭니다.'
		],
		explain:
			'알파벳 순번으로 보면 ALMOST(1,12,13,15,19,20)·BEGIN(2,5,7,9,14)·CHIMP(3,8,9,13,16)·GHOST(7,8,15,19,20)·EMPTY(5,13,16,20,25)는 전부 <b>계속 커지기만</b> 합니다. <b>GIFT</b>만 G(7)→I(9)→F(6)에서 줄어듭니다.'
	},
	{
		id: 'weekday-batchim',
		chip: '받침',
		blocks: [
			{ kind: 'text', html: '아래 세 요일에는 공통점이 있다. 같은 공통점을 가진 요일이 <b>하나 더</b> 있다.' },
			{ kind: 'pre', text: '월요일\n목요일\n금요일' },
			{ kind: 'text', html: '남은 하나는 무슨 요일일까?' }
		],
		type: 'text',
		answers: ['일', '일요일'],
		hints: [
			'요일 순서도, 한자 뜻(月木金)도 아닙니다. <b>소리 내어</b> 읽어 보세요.',
			'월, 목, 금 — 끝소리를 막는 자음이 하나씩 붙어 있습니다.',
			'일곱 요일 중 받침이 있는 건 딱 넷뿐입니다. 화·수·토는 없습니다.'
		],
		explain:
			'월(ㄹ)·목(ㄱ)·금(ㅁ)은 모두 <b>받침이 있는</b> 요일입니다. 화·수·토는 받침이 없죠. 남은 하나는 <b>일요일</b>(ㄹ 받침).'
	},
	{
		id: 'subway-board',
		chip: '전광판',
	...
	},
	{
		...
		chip: '달력',
		blocks: [
			{ kind: 'text', html: '어느 해의 달들을 기록해 두었다.' },
			{
				kind: 'pre',
				text: '1월 : 1일 화요일  →  31일 목요일\n4월 : 1일 화요일  →  30일 수요일\n2월 : 1일 목요일  →  28일 수요일'
			},
			{
				kind: 'text',
				html: '어떤 달은 1일이 <b>일요일</b>, 마지막 날이 <b>토요일</b>이었다. 이 달은 며칠까지 있었을까?'
			}
		],
		type: 'text',
		answers: ['28', '28일'],
		hints: [
			'달 이름을 외우는 문제가 아닙니다. 1일과 마지막 날 <b>사이의 간격</b>을 세어 보세요.',
			'1월은 화→목으로 2칸, 4월은 화→수로 1칸, 2월은 목→수로 6칸 밀렸습니다.',
			'밀린 칸 수는 (그 달 일수−1)을 7로 나눈 나머지입니다. 일→토는 6칸이니, 일수는 7의 배수여야 합니다.'
		],
		explain:
			'요일이 밀린 칸 수 = (일수−1)을 7로 나눈 나머지. 1월 31−1=30 → 나머지 2(화+2=목 ✓), 4월 29 → 1(화+1=수 ✓), 2월 27 → 6(목+6=수 ✓). 일→토는 <b>6칸</b>이니 일수−1이 7의 배수 → 일수는 28·35… 달의 길이는 28~31이므로 답은 <b>28</b>일.'
	}
];
