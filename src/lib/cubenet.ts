...
/**
 * 서로 다른 전개도 그림의 가짓수 = 전개도 모양 × 기호 배치(6!).
 * 전개도는 저장된 문제가 아니라 번호를 넣으면 만들어지므로, 다른 유형처럼 "몇 개"가 없다.
 * 화면에 개수를 보여줄 때는 이 값을 쓴다(같은 그림도 묻는 각도가 24가지라 실제 문제는 더 많다).
 */
export const CUBE_VARIANTS = NETS.length * 720;
