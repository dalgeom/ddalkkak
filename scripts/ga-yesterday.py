...
ALERT = [
    # 9/15부터 알림 카드 안에 내일 발견형 문제의 첫 예시 한 줄이 붙었다(PushPrompt tomorrow).
    # 그 전 8/01~9/14는 제안 25명 → 켬 3명(12%). 날짜로 나눠 이 비율과 비교한다.
...
CTA = [
    ...
    # 9/14부터 버튼을 누르면 홈 소개를 건너뛰고 곧장 시작한다. 이 줄이 클릭 수에 가까워야 한다
    ("cta_go", "버튼으로 와서 곧장 시작으로 넘어감"),
]
