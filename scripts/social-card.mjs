 ...
 */
const CARDS = {
	// 처음엔 word-overlap(LEMON + ONION = LEMONION)으로 만들었다가 올리기 전에 갈았다 — 사용자가
	// 「중복 글자 한 번만 쓰는 게 1초 만에 보이는 쓰레기 문제」라고 했다. 합친 단어가 두 단어를 눈에
	// 보이게 품고 있어 3초 룰에 걸린다(9/07 kr-loan-stretch와 같은 병). 「죽는 줄이 있나」만 보고 골랐다.
	// rc-seat-farthest는 「반씩 가르기」가 8개 방 4번째에서, 「긴 빈칸 가운데」가 10개 방 4번째에서 죽고,
	// 규칙(가장 가까운 사람과 가장 멀리, 같으면 왼쪽)을 시뮬레이션해야 보인다. 데일리 첫 출제는 12/24.
	'2026-09-15': {
		id: 'rc-seat-farthest',
		chip: '번호 매기기',
		물음: '의자 12개 방에서 4번째로 온 사람은 왼쪽에서 몇 번째에 앉을까?',
		// 빈 의자를 _로 두면 12개 줄이 이어진 선처럼 보여 칸을 셀 수 없었다 — □로 한 칸씩 보이게 한다
		줄: ['숫자 = 들어온 순서 · □ = 빈 의자', '7개   1 □ □ 3 □ □ 2', '8개   1 □ □ 3 □ 4 □ 2', '10개  1 □ 4 □ 3 □ 5 □ □ 2', '12개  1 □ □ □ □ 3 □ □ □ □ □ 2']
	},
	...
	'2026-09-14': {
		...
		chip: '숫자의 정체',
		물음: '물음표에 들어갈 수는?',
		줄: ['33511 → 3', '7777 → 1', '1212 → 4', '188229 → ?']
	},
	'2026-09-11': {
	...
	'2026-09-10': {
		...
		chip: '이상한 연산',
		물음: '물음표에 들어갈 수는?',
		줄: ['9 ※ 8 = 17', '5 ※ 3 = 16', '7 ※ 2 = 45', '6 ※ 2 = 32', '8 ※ 6 = ?']
	},
	// 처음엔 diamond-op(곱한 뒤 뒤집기)로 찍었다가 물렀다 — 8/11 num-mult-flip으로
	// **똑같은 메커니즘**이 이미 나갔다(사용자가 잡았다). id가 달라 스크립트가 못 걸렀고,
	// num-mult-flip은 그 뒤 은행에서 교체돼 흔적조차 없었다. thread-post.mjs에 chip별
	// 사용 이력을 찍게 고쳤다. 「이상한 연산」은 이미 세 번 나가 결도 바꿨다.
	'2026-09-09': {
		id: 'num-coin-count',
		chip: '수의 성질',
		물음: '물음표에 들어갈 수는?',
	...
	'blog-격간': {
		...
		chip: '사슬',
		물음: '자리를 하나씩 건너뛰어 읽으면',
		// 마지막 줄이 주황으로 강조된다 — 물음표가 있는 홀수 자리를 끝에 둔다
		줄: ['27  →  20  →  18  →  10  →  ?', '짝수 자리 : 20 · 10', '홀수 자리 : 27 · 18 · ?']
	},
	'blog-사슬': {
		id: 'rc-chain-selfrotate',
		chip: '사슬',
		물음: '앞의 수로 다음 수를 만든다',
		줄: ['33412 → 12334 → 23341 → 34123 → ?']
	},
	'blog-한자': {
		id: 'glyph-cross-junction',
		chip: '교차점',
		물음: '물음표에 들어갈 수는?',
		// 마지막 줄만 주황이라 물음표 줄을 혼자 둔다 — 申 = 3이 같이 강조되면 예시가 질문처럼 보인다
		줄: ['日 = 0    田 = 1    井 = 4', '土 = 1    申 = 3', '由 = ?']
	},
	'2026-09-08': {
		id: 'hidden-clock-add',
		chip: '숨은 표기법',
		물음: '물음표에 들어갈 수는?',
		줄: ['215 + 30 = 245', '145 + 30 = 215', '250 + 15 = 305', '320 + 50 = ?']
	},
	// 처음엔 kr-loan-stretch로 찍어 올렸다가 내렸다 — 예시에 「MILK → 밀크 (2글자)」처럼
	// 한글 표기가 그대로 적혀 있어 발견할 것이 없었다(사용자 지적). 문제는 고쳤고
	// puzzle-reviewer에 게이트 5m을 넣었다. 카드는 이걸로 다시 찍었다.
	'2026-09-07': {
		id: 'rc-chain-maxdigit',
		chip: '사슬',
		물음: '물음표에 들어갈 수는?',
	...
	'blog-제작-키패드': {
		...
		chip: '숫자의 정체',
		물음: '회원이 될 수 있는 수는?',
		줄: ['회원 : 123  456  951  2580', '거절 : 246  124  137  426', '후보 : 741  953  357  148']
	},
	'blog-제작-책벌레': {
		id: 'ob-bookworm',
		chip: '생활 상식',
		물음: '벌레가 지나간 거리는?',
		줄: ['1권 2권 3권이 나란히 꽂혔다', '본문 4cm · 표지 각 0.5cm', '1권 첫 쪽 → 3권 마지막 쪽', '몇 cm를 지났을까?']
	},
	'blog-제작-한붓': {
		id: 'word-pen-lift',
		chip: '관찰',
		물음: 'PLANE은 얼마일까요?',
		줄: ['VAN = 4      CAB = 4', 'BUS = 3      BOAT = 6', 'YACHT = 10', 'PLANE = ?']
	},
	'blog-함정-방망이': {
		id: 'ob-bat-ball',
		chip: '함정',
...
function 폭(s) {
	...
	for (const ch of s) {
		...
		// 대문자는 소문자·숫자보다 넓다. 0.58로 치면 9/15 「LEMON + ONION = LEMONION」이 상자 밖으로 나갔다
